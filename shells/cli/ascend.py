...
def _assess_run_many(args, c, refs, scope_ids=None):
    ...
    Each run is created with wait=False so they start together; `_wait_for_fleet` then follows
    all of them to a terminal state and prints one summary. A fleet that returned immediately
    reported three runs as "running" and exited 0 — a pipeline read that as a passing gate before
    a single probe had been answered. (This wait landed in #52 and was silently reverted by a bad
    merge commit; restored.)
...
def cmd_assess_results(args):
    ...
    if st not in api.TERMINAL_STATUSES and not args.json:
        ...
        # (Restored: this landed in #52 and was silently reverted by a bad merge commit.)
        prog = a.get("progress")

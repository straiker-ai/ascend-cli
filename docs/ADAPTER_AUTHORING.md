...
| `conversation` | `direct_api`, `session_api` | `{"policy": "per-probe" or "sequential", "max_turns": 10}` -- per-probe (default): each probe is its own conversation. sequential: consecutive probes share one, bounded by `max_turns`. Set per run with `assess run --conversation` |
| `carry` | `direct_api` | `{"reply_path": "conversation_id", "request_field": "conversation_id"}` — echo the conversation id the target returns in each reply on the next request. The adapter instance keeps the latest one; a target that rotates ids (the old one 409s) or closes a conversation after N turns (410) gets one retry without the id, which opens a fresh conversation. `target add` records this when the winning reply carries such a field and then proves a second turn with it is accepted. Honoured only under `conversation.policy: sequential` -- per-probe (the default) never carries an id across a probe boundary |

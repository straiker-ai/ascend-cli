...
- **Any number of environment-referenced credentials per target.** A gateway passcode header AND
  a bearer is the ordinary field case; a public demo host behind exactly that pair was refused with
  "one environment-referenced credential per target". `target add` now records every `env:`
  reference, the config's `auth` becomes a list past one credential, and the relay resolves each
...
- **`docs/QA_COVERAGE.md`** -- every command path and how it was last proven to work: live by an
  ...
  API ships), or by policy (`tenant switch`). Generated by `scripts/qa_coverage.py`; a command
  that is only unit-tested says so, because an offline test is a regression guard, not proof.
- **`scripts/live_matrix_record.py --base-port`** so the post-merge matrix can run while another
  target is under test on the default ports.
...
- **The request after creating a run no longer inherits a dead connection.** The platform closes
  ...
  before the close was visible, and a POST is rightly never retried. It surfaced as
  `recovered: true … the connection dropped (ConnectionError) after the assessment was created` on
  12 of 28 real runs in a measured round (timing-dependent), and on 3 of 3 when the sequence is
  replayed without a pause. The create now sends `Connection: close`, so the transitions open a
  fresh connection, and a transition that meets a no-response drop is retried once -- it is
  409-tolerant, so a request that did land is harmless to repeat. Recovery notes, when they do
  fire, now name the underlying error rather than just its class.

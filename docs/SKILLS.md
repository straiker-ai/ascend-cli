...
Three layers, one core, no logic duplication:
...
- **Deterministic core (the CLI).** Every verb takes `--json` and emits a stable
  ...
  transitions, adapter classification, the CI gate — all live here, all tested,
  all repeatable. The core never guesses.
- **Skills (the reasoning layer).** `SKILL.md` workflows that orchestrate CLI
  verbs — they choose, sequence, and interpret, but never reimplement CLI logic.
...
- **The agent (the driver).** Any host executes the skills against the CLI —
  Claude Code, Cursor, Codex, a plain terminal. For shell-less hosts (claude.ai
...
`agent/` is the **judgment layer**: prompt material, not code. Nothing in it is
imported, run, or affects a CLI number. It is where the calls the CLI *refuses to
guess* are written down for a human or agent to apply with the evidence in front
of them — chiefly false-positive triage (`agent/TRIAGE.md`). The CLI does the
arithmetic; `agent/` holds the judgment.

## The skill set grows with the platform

The skills are not a fixed set. As the Straiker platform APIs add capabilities, the
...
Inputs to **build-adapter** — HAR export, curl/proxied send, spec, or live URL —
all enter through `ascend discover`; nothing ships until `ascend adapter validate`
...
- `ascend bridge sync` reconciles bridges to assessment state (start for
  running/paused apps, stop for terminal) — the manual fallback when state changed
  in the Console.
...
an opaque `request_id`/`msg_id` that the bridge echoes back; the platform
...
## Repo layout — where capabilities live
...
| `agent/` | judgment prompts (`TRIAGE.md`, `README.md`) — prompt material, not code |
